...
int ParseCmd(unsigned char *buffer, uint8_t len)
{
    ...
    {
        EraseAP(FMC_APROM_BASE + APROM_APPLICATION_START, (g_apromSize < g_dataFlashAddr) ? g_apromSize : g_dataFlashAddr); // erase APROM // g_dataFlashAddr, g_apromSize
    ...
    if ((lcmd == CMD_UPDATE_APROM) || (lcmd == CMD_UPDATE_DATAFLASH))
    {
        ...
        {
            StartAddress = APROM_APPLICATION_START;   //StartAddress = 0;
        }
